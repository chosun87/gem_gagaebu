...
- Routing URL 2. 통계 : /gem_gagaebu/statistics 3. 자산 : /gem_gagaebu/asset 4. 설정 : /gem_gagaebu/settings 5. login : /gem_gagaebu/login
...
3. 다이얼로그 컴포넌트 (DialogTheme, DialogSettings, DialogLedger, DialogAI, DialogRepeat, DialogList) 등도 React.lazy를 사용하여 다이나믹 로딩을 적용했습니다.

- 다이얼로그 지연 로딩: 이제 앱이 처음 켜질 때 이 컴포넌트들을 메인 번들에 포함시키지 않고, 사용자가 팝업이 필요한 시점에 코드를 불러옵니다.
...
1. 유틸리티 함수 분리 (src/utils/dataUtils.js 신규 생성)

- parseAmount(val): 여러 Context에 흩어져 있던 복잡한 금액 문자열 파싱 정규식을 하나로 통일했습니다.
- calculateRepeatDates(repeat): 무려 130줄이 넘어가던 generateLedgerFromRepeat 내부의 핵심 반복 날짜 계산 로직(매월/매주)을 순수 함수로 빼내어 분리했습니다. 코드가 훨씬 가벼워지고 유지보수가 편해졌습니다.

2. 구글 시트 일괄 업데이트 (Batch Append) API 추가

- src/api/sheetApi.js에 appendSheetRows 함수를 새로 추가했습니다.
- 가계부 내역 자동 생성 시, 기존에는 for문을 돌며 한 줄씩 통신해서 속도가 느렸지만, 이제는 새 행 배열(newRows)을 모아 단 1번의 통신으로 시트에 수십 개의 데이터를 한 번에 반영합니다.

3. 즉각적인 반영을 위한 낙관적 UI 업데이트 (Optimistic UI Update)

- DataContextYYYY.jsx, DataContext반복.jsx의 저장/수정/삭제 함수(saveLedgerEntry, saveRepeatEntry, deleteLedgerEntry, deleteRepeatEntry) 모두 적용했습니다.
- 기존의 "시트에 저장 완료 후 다시 전체 다운로드(loadSheet연도Data() 등)" 하는 과정을 없앴습니다.
- 새로운 로직: 시트에 저장한 즉시, 브라우저가 들고 있는 State 배열(sheetYYYYData, sheet반복Data)에 값을 바로 수정하거나 추가합니다. 이렇게 하면 로딩 스피너 도는 시간이 현저히 줄어들고 클릭 즉시 데이터가 화면에 반영되는 것을 체감하실 수 있습니다. (물론 시트 통신은 비동기로 안전하게 뒷단에서 처리됩니다.)

[전체 리팩토링]
🛠️ 전체 소스 리팩토링 내역

1. Context Provider 성능 최적화 완료

- DataContextYYYY.jsx, DataContext반복.jsx, DataContext자산.jsx, DataContext코드.jsx 4개 파일의 하단 value 객체 생성부에 모두 useMemo를 적용했습니다.
- 내부에 존재하는 saveLedgerEntry, deleteRepeatEntry 등의 모든 함수들을 useCallback으로 감싸서 메모이제이션 했습니다.
- 부모인 DataContext.jsx에서 반환하는 합산 데이터(useData()) 결과도 useMemo로 캐싱했습니다.
- 효과: 가계부 내역 하나를 조작하거나 탭을 이동할 때 일어나는 불필요한 연쇄 리렌더링이 대폭 감소하여 앱이 훨씬 부드럽게 동작합니다.

2. 모바일 스와이프 전용 Custom Hook 분리

- src/hooks/useSwipe.js 파일을 신규 생성했습니다.
- Calendar.jsx와 MonthlyList.jsx 안에 지저분하게 남아있던 onTouchStart, onTouchEnd, 거리 계산 로직을 모두 지우고 useSwipe 훅 하나로 대체했습니다.
- 효과: UI 렌더링 파일에 순수하게 뷰(View)에 관련된 코드만 남게 되어 유지보수성과 가독성이 크게 좋아졌습니다.

3. 비즈니스 로직 Service & Utility 계층 분리

- 130줄이 넘어가던 generateLedgerFromRepeat 내부 로직과 구글 시트 다중 추가(appendSheetRows) 로직을 src/utils/dataUtils.js 와 sheetApi.js 로 완전히 분리시켰습니다.

4. 하드코딩된 문자열 (Magic String) 상수화

- src/assets/js/constants.js 파일에 TRANSACTION_TYPE (수입/지출/이체), REPEAT_PERIOD (월간 M / 주간 W) 상수를 새로 선언했습니다.
- Repeat.jsx의 정렬 로직과 UI 출력 조건, 그리고 dataUtils.js의 내부 로직에 하드코딩되어 있던 'M', 'W' 문자들을 모두 상수로 변경했습니다.
- 효과: 이후 기획이나 데이터 구조가 변경되더라도 constants.js 한 곳만 수정하면 전체 앱에 안전하게 반영되며, 오타로 인한 버그가 차단됩니다.
