...
export default function Statistics() {
  const { yearData, selectedDate, setSelectedDate, categoryOptions } = useData();
  ...
  const { chartData, chartOptions, listData, totalAmount } = useMemo(() => {
    ...
    const monthData = (yearData || []).filter(item => {
      ...
      return d.year() === selectedDate.getFullYear() && d.month() === selectedDate.getMonth();
    });
    ...
    // 따뜻하고 조화로운 색상 팔레트
    const colors = [
      '#e87461', '#f5a65b', '#f9d266', '#c4e082', '#97d296', 
      '#7ac4cb', '#8cb2de', '#a69cd9', '#c98fc9', '#e685a6', '#a0a0a0'
    ];
    ...
    const sortedCategories = Object.keys(categoryTotals).map(cat => {
      ...
      const catNode = categoryOptions.find(group => group.cdGroup === '지출')?.children?.find(c => c.cd === cat);
      return {
        id: cat,
    ...
    return { chartData: data, chartOptions: options, listData: sortedCategories, totalAmount: totalExpense };
    ...
    <div className="flex flex-column h-full w-full bg-white">
      ...
      <div className="flex justify-content-center align-items-center p-3 surface-card border-bottom-1 surface-border">
        <div className="flex align-items-center gap-1">
          <Button icon="pi pi-angle-left" text rounded severity="secondary" onClick={handlePrev} />
          <span className="text-xl font-bold px-4">{dayjs(selectedDate).format('YYYY년 M월')}</span>
          <Button icon="pi pi-angle-right" text rounded severity="secondary" onClick={handleNext} />
        </div>
      </div>
      ...
      <div className="flex-1 overflow-auto p-3 surface-ground">
        {totalAmount > 0 ? (
          <div className="bg-white border-round shadow-1 p-3 mb-4">
            ...
            {/* 차트 영역 */}
            <div className="flex justify-content-center mb-5" style={{ height: '280px' }}>
              <Chart type="pie" data={chartData} options={chartOptions} className="w-full flex justify-content-center" />
            </div>

            {/* 리스트 영역 */}
            <div className="border-top-1 surface-border pt-3">
              {listData.map((item) => (
                <div key={item.id} className="flex align-items-center py-3 border-bottom-1 surface-border">
                  <div className="flex align-items-center gap-3 flex-1">
                    <span className="border-round text-white font-bold px-2 py-1 text-sm text-center" style={{ backgroundColor: item.color, minWidth: '45px' }}>
                  ...
                  <div className="font-bold text-lg text-800">
            ...
            <div className="flex justify-content-end mt-4 p-3 surface-ground border-round">
              <div className="text-xl font-bold text-800 flex align-items-center gap-3">
                <span>지출</span>
                <span className="text-primary text-2xl">{totalAmount.toLocaleString()}원</span>
              </div>
            </div>
  ...
  return (
    <div className="app-page statistics-page">
      <TabView className="ledger-tabview" activeIndex={activeIndex} onTabChange={(e) => setActiveIndex(e.index)}>
        <TabPanel header={<span className="text-lg">주간</span>} className="p-0">
          {renderContent()}
        </TabPanel>
        <TabPanel header={<span className="text-lg">월간</span>} className="p-0">
          {renderContent()}
        </TabPanel>
        <TabPanel header={<span className="text-lg">연간</span>} className="p-0">
          {renderContent()}
        </TabPanel>
      </TabView>
    </div>
