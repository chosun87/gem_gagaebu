...
export default function MonthlySummaryChart지출({ months, rawData }) {
  ...
  const getThemeColor = (colorName, shade) => {
    const varName =
      colorName === 'primary' ? `--primary-${shade}` : `--${colorName}-${shade}`
    return (
  ...
  const chartOptions = {
    ...
    plugins: {
      ...
      tooltip: {
        ...
        intersect: false,
        itemSort: (a, b) => {
          const orderA = categoryMap[categoryList[a.datasetIndex]]?.cdOrder || 0
          const orderB = categoryMap[categoryList[b.datasetIndex]]?.cdOrder || 0
